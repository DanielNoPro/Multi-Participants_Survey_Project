...
from tenant.views import (
    ReadOnlyConfigurationView,
    ReadOnlyServiceView,
    ReadOnlyTenantView,
    ReadOnlyUnitView,
    WriteOnlyConfigurationView,
    WriteOnlyServiceView,
    WriteOnlyTenantView,
    WriteOnlyUnitView,
)
...
write_tenants_units_router = routers.NestedSimpleRouter(
    write_tenants_router, r"tenants", lookup="tenant"
)
write_tenants_units_router.register(
    r"tenants", WriteOnlyUnitView, basename="Unit write"
)

read_tenants_router = routers.SimpleRouter()
read_tenants_router.register(r"tenants", ReadOnlyTenantView, basename="Tenant read")

read_tenants_units_router = routers.NestedSimpleRouter(
    read_tenants_router, r"tenants", lookup="tenant"
)
...
write_services_router.register(
    r"services", WriteOnlyServiceView, basename="Service write"
)
...
write_configs_router.register(
    r"configs", WriteOnlyConfigurationView, basename="Config write"
)

read_configs_router = routers.SimpleRouter()
read_configs_router.register(
    r"configs", ReadOnlyConfigurationView, basename="Config read"
)
